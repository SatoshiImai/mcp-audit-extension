...
describe('AuditHost L2 policy', () => {
  ...
  it('does not flag a fail-closed aborted outcome under L2 as a sequence gap (§10.4)', () => {
    ...
    expect(host.handleAttempt(signEvent(attempt(1), key.keyId, 0, key.privateKey)).status).toBe('unavailable');
    host.unavailable = false;
    ...
    const aborted = signEvent({ ...attempt(1), outcome: 'aborted', reason: 'host-unavailable' }, key.keyId, 1, key.privateKey);
