...
describe('capability negotiation (§6.1)', () => {
  ...
  it('a signing host satisfies a tool that requires a countersign (§5.2)', () => {
    expect(negotiateCapability(COUNTERSIGNING, COUNTERSIGNING).negotiated).toBe(true);
  });

  it('a non-signing host cannot satisfy a tool that requires a countersign', () => {
    const result = negotiateCapability(TOOL_L1, COUNTERSIGNING);
    expect(result.outcome).toBe(MISMATCH);
  ...
  it('the axes run in opposite directions', () => {
    ...
    expect(negotiateCapability(COUNTERSIGNING, TOOL_L1).negotiated).toBe(true);
    expect(negotiateCapability(TOOL_L1, COUNTERSIGNING).negotiated).toBe(false);
  });
