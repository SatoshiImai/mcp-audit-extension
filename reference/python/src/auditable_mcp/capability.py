...
tool offering L2 satisfies an L1 host. On `witness` (§5.2) the host produces and the tool requires,
...
class AuditCapability:
    ...
    witness: str = 'none'  # 'none' | 'host' - the host offers it, the tool requires it (§5.2)
...
def negotiate_capability(host: AuditCapability | None, tool: AuditCapability) -> NegotiationResult:
    ...
    if host is None:
        return NegotiationResult(
            tool=tool, host=None, outcome=UNDECLARED, version_match=False, level_fit=False, witness_fit=False
        )
