...
def test_a_signing_host_satisfies_a_tool_that_requires_a_witness() -> None:
    """The witness axis runs the other way: the host offers, the tool requires (§5.2)."""
    assert negotiate_capability(_WITNESSING, _WITNESSING).negotiated is True
...
def test_a_non_signing_host_cannot_satisfy_a_tool_that_requires_a_witness() -> None:
    ...
    result = negotiate_capability(_TOOL_L1, _WITNESSING)
    assert result.outcome == MISMATCH
...
def test_the_axes_run_in_opposite_directions() -> None:
    """A tool surplus is safe on `level`; a host surplus is safe on `witness`."""
    ...
    assert negotiate_capability(_WITNESSING, _TOOL_L1).negotiated is True
    assert negotiate_capability(_TOOL_L1, _WITNESSING).negotiated is False
