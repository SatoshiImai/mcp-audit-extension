...
def test_fail_closed_aborted_not_flagged_as_sequence_gap() -> None:
    ...
    host, key = _new_host()
    host.unavailable = True
    assert host.handle_attempt(sign_event(_attempt(1), key.key_id, 0, key.private_key)).status == 'unavailable'
    host.unavailable = False
    event = {**_attempt(1), 'outcome': 'aborted', 'reason': 'host-unavailable'}
    host.handle_outcome(sign_event(event, key.key_id, 1, key.private_key))
